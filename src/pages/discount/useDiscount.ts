...
const useDiscount = () => {
    ...
    switch (action) {
      ...
      case "UPDATE": {
        const updatedData = data.map((item: Discount) =>
          item.discountId === discount.discountId ? discount : item
        );
        setData(updatedData);
        break;
      }
      case "DELETE": {
        setData((prevData) =>
          prevData.filter((item) => item.discountId !== discount.discountId)
        );
  ...
  const editDiscount = (discountId: string, discount: Discount) => {
  ...
  const deleteDiscountById = async (discountId: string) => {
    ...
    try {
      ...
      if (response) {
        setDiscountData({ discountId } as Discount, "DELETE");
      }
    } catch (e) {
      console.error("An error occurred while deleting the discount", e);
    } finally {
      clearForm();
      setLoading(false);
    }
